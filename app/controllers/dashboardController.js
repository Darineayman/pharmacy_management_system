app.controller("DashboardCtrl", function ($scope, $q, dashboardApi, invoicesApi, invoiceItemsApi, medicinesApi) {
    var vm = this;
    ...
    var invoiceDetailsModalInstance = null;

    vm.formatCompactCurrency = function (value) {
        var amount = Number(value || 0);
        var abs = Math.abs(amount);

        function trim(valueText) {
            return valueText.replace(/\.0+$/, "").replace(/(\.\d*[1-9])0+$/, "$1");
        }

        if (abs >= 1000000000) {
            return "$" + trim((amount / 1000000000).toFixed(2)) + "B";
        }

        if (abs >= 1000000) {
            return "$" + trim((amount / 1000000).toFixed(2)) + "M";
        }

        if (abs >= 1000) {
            return "$" + trim((amount / 1000).toFixed(2)) + "K";
        }
            ...
            .then(function (results) {
                ...
                vm.topMedicines = topMedicines.map(function (med, index) {
                    ...
                    return {
                        ...
                        amount: vm.formatCompactCurrency(med.total),
                        height: scaledHeight,
    ...
    vm.showInvoiceModal = function () {
        var el = document.getElementById("dashboardInvoiceDetailsModal");
        invoiceDetailsModalInstance = bootstrap.Modal.getOrCreateInstance(el);
        invoiceDetailsModalInstance.show();
    };

    vm.openViewInvoiceModal = function (order) {
        vm.invoiceView.loading = true;
        vm.invoiceView.error = "";
        vm.invoiceView.invoice = {
            invoice_number: order.id,
            customer_name: order.customer,
            status: order.status,
            total_amount: Number(String(order.price || "0").replace(/[^0-9.-]/g, ""))
        };
        vm.invoiceView.items = [];

        $q.all([
            invoiceItemsApi.getByInvoiceId(order.invoiceId),
